...
与音频文件同名的 `.json`（例如 `clip.wav` 对应 `clip.json`）会自动配对，并显示在对应卡片**右侧的元数据面板**中；对于 diff 组，则显示当前活动 lane 的 JSON。

- 点击任一 **JSON 键** 进行多选，可跨所有已加载文件横向比较该字段的值
- **fields** 按钮打开可搜索的字段浏览器，列出所有已加载文件中出现的字段
- 拖动面板分隔条调整宽度（双击复位）
- 工具栏 **JSON** 开关控制所有面板的显示/隐藏。**没有**对应音频的 `.json` 文件会在开关打开时显示为**独立 JSON 卡片**。

### 多声道播放

立体声及多声道文件（2–8 通道）会展开为**每个通道一个 lane**，各带独立的波形、频谱与频率轴。播放时默认**混合全部通道**（立体声文件的左右通道都会在两耳听到）。每个 lane 上有 **Mute** 按钮可静音该通道，支持任意组合同时静音。超过 8 通道的文件回退为单个混音视图。
